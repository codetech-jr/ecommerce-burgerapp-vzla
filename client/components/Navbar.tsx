...
import { MapPin, Menu, X, Bike } from "lucide-react";
...
export default function Navbar() {
  ...
  useEffect(() => {
    // Solo en el cliente, verificamos si hay una orden activa guardada
    const id = localStorage.getItem("burgerapp_last_order");
    if (id) setActiveOrderId(id);
  }, [pathname]); // Re-chequear al navegar
      ...
      <nav className="sticky top-0 z-50 w-full bg-white/95 backdrop-blur-md border-b border-slate-100 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 h-16 sm:h-20 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            {activeOrderId && (
              <Link href={`/order-success?id=${activeOrderId}`} className="hidden sm:flex items-center gap-2 bg-orange-100/50 hover:bg-orange-100 text-orange-700 px-3 py-1.5 rounded-full border border-orange-200 transition-colors text-xs font-bold font-heading shadow-sm">
                <Bike size={14} className="animate-bounce" /> Mi Pedido
              </Link>
            )}
